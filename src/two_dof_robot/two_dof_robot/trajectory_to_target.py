...
class TrajectoryToTarget(Node):
    def __init__(self):
        ...
        # Kol uzunlukları (URDF'ten)
        self.l1 = 1.0
        self.l2 = 1.0

        # Kontrol modu (şimdilik sadece "ik" kullanılıyor)
        self.control_mode = "ik"

        if self.control_mode == "ik":
            # Home ve Target konumları (uç efektör pozisyonları)
            home_x = 0.0
            home_y = 0.0
            ...
            raise NotImplementedError("Sadece 'ik' modu destekleniyor.")

        # Sonsuz döngü için başlangıç ve hedef açıları
        self.q0 = self.home_q
    ...
    def publish_trajectory(self):
        ...
        if self.elapsed_time > self.T:
            # Hedefe ulaşıldı → q0 ve qf'yi değiştir
            self.q0, self.qf = self.qf, self.q0
            self.elapsed_time = 0.0
            self.get_logger().info("Target reached. Reversing direction...")
