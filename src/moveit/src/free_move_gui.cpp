...
class FreeMoveGui : public QWidget
{
  Q_OBJECT
public:
  FreeMoveGui(QWidget* parent=nullptr) : QWidget(parent)
  {
    node_ = std::make_shared<rclcpp::Node>("free_move_gui");
    ...
    z_ = new QDoubleSpinBox(); z_->setRange( 0.0,  2.0); z_->setDecimals(4); z_->setSingleStep(0.01); z_->setValue(0.80);
    ...
    auto* send = new QPushButton("Gönder (Free Move)");
    v->addWidget(send);
...
int main(int argc, char** argv)
{
  ...
  FreeMoveGui w;
  w.setWindowTitle("Free Move (x,y,z) -> /vector_distance_input");
  w.show();
